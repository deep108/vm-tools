...
- Ruby via mise — precompiled binary install on ARM64 Linux, configured in `~/.config/mise/config.toml` with `ruby = "3.4"`. Setting `[settings.ruby] compile = false` opts into precompiled binaries explicitly so we don't depend on mise's default flipping in a future version (becomes default in mise 2026.8.0).
- `kamal` gem at pinned version (currently 2.11.0), installed via `gem install --user-install` in `run_onchange_before_05-install-kamal.sh.tmpl`. Bumping just means editing `KAMAL_VERSION` in the script — `run_onchange_` re-triggers automatically.
...
- `build-essential` — for native gem extensions; already installed by `bootstrap-linux.sh` before Homebrew, since linuxbrew itself requires a working compiler to bootstrap.
